...
function pickBinaryAssetForPlatform({ platform, arch, zipNames }) {
    ...
    if (platform === 'win32') {
        const winCandidates = candidates.filter((name) => /win-(vulkan|cpu|avx2?|avx512|noavx|cuda12|cu12)-x64/.test(name));
        const order = ['win-vulkan-x64', 'win-cpu-x64', 'win-avx2-x64', 'win-avx-x64', 'win-cuda12-x64', 'win-cu12-x64'];
